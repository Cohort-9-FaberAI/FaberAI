...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const initials = (userEmail?.trim().charAt(0) ?? 'G').toUpperCase()
    ...
    <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <div style={{ marginTop: 'auto' }}>
        <div className="sidebar-account">
          <span className="sidebar-account-avatar" title={userEmail ?? 'Guest'}>
            {initials}
          </span>
          <div className="sidebar-account-info">
            <strong title={userEmail ?? undefined}>{userEmail ?? 'Guest'}</strong>
            <span>{userEmail ? 'Signed in' : 'Not signed in'}</span>
          </div>
          <NavLink
            to="/settings"
