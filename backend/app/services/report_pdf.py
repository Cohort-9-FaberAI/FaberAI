...
def _svg_to_pdf(path_str: str, x0: float, y0: float, scale: float, view_height: float = 1080.0) -> list[str]:
    ...
    while i < len(tokens):
        ...
        if active_cmd in ('m', 'M'):
            ...
            i += 6
            if active_cmd == 'c':
                x1 += cur_x; y1 += cur_y; x2 += cur_x; y2 += cur_y; x3 += cur_x; y3 += cur_y
            px1, py1 = to_pdf(x1, y1); px2, py2 = to_pdf(x2, y2); px3, py3 = to_pdf(x3, y3)
            ...
            i += 4
            if active_cmd == 'q':
                qx += cur_x; qy += cur_y; x3 += cur_x; y3 += cur_y
            x1 = cur_x + 2.0 / 3.0 * (qx - cur_x); y1 = cur_y + 2.0 / 3.0 * (qy - cur_y)
            x2 = x3 + 2.0 / 3.0 * (qx - x3); y2 = y3 + 2.0 / 3.0 * (qy - y3)
            px1, py1 = to_pdf(x1, y1); px2, py2 = to_pdf(x2, y2); px3, py3 = to_pdf(x3, y3)
            cmds.append(f"{px1} {py1} {px2} {py2} {px3} {py3} c")
            cur_x, cur_y = x3, y3
        else:
