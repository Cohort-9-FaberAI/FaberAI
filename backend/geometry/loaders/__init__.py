"""
File loading for the geometry spine.

load_geometry() is the single entry point the rest of FaberAI should call —
it dispatches on file extension and always returns a populated GeometryModel,
regardless of whether the source was a STEP or an STL file.
"""
...
def load_geometry(path: str) -> GeometryModel:
    ...
    if ext in STEP_EXTENSIONS:
        ...
        model.measurements_reliable = is_mesh_reliable(mesh)  # check AFTER repair attempt
        return model
